...
def machine_diff(obj1: Any, obj2: Any, root='', verbose=False) -> bool:
    ...
    for key, value in p1.items():
        ...
        match value.__class__.__name__:
            case ('Coils' | 'Shells' | 'Separatrices' | 'Measures' |
                  'Limiters' | 'SubCoils' | 'SubShells'):
                for i, coil in enumerate(value):
                    areDiff |= machine_diff(coil, getattr(obj2, key)[i],
                                          root=f"{root}.{key}[{i}]")
            case ('Coil' | 'Shell' | 'Separatrix' | 'Measure' | 'Limiter' |
                  'SubCoil' | 'SubShell' | 'Plasma' | 'PsiGrid' | 'Machine'):
            ...
            case 'VectorView' | 'MatrixView' | 'IMatrixView':
                s1, s2 = (np.array(value).shape, 
                          np.array(getattr(obj2, key)).shape)
                if s1 != s2:

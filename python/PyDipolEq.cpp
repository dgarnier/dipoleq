...
// because nrutils suck and you cannot just free a thing
...
// free_SUBCOIL not needed
// free_LIMITER not needed
// free_SEPARATRIX not needed
...
get_flux_contour(PSIGRID *pg, double psi) {
    ...
    // these suck to get rid of.. So, cheat and put
...
PYBIND11_MODULE(core, m) {
    ...
    // pybind11-stubgen cannot handle forward declarations
    ...
    py::class_<PSIGRID>(m, "PsiGrid", "Magnetic flux grid data and methods")
        ...
        .def("get_new_integrand", [](PSIGRID &self) -> py::object
             { return py::cast(new DMatrixView(self.Nsize)); }, "Allocate matrix to itegrate over.")
        .def("near_plasma", [](PSIGRID &self, int ir, int iz) -> bool
             { return (bool)NearPlasma(self.IsPlasma, ir, iz); }, "Check if a grid point is near the plasma")
        .def("contour_integral", [](PSIGRID &self, DMatrixView &integrand, double psiX, bool average = false) -> double
             { return DoFluxContourIntegral(&self, integrand.dmatrix(), psiX, average); }, "Do a flux contour integral");
    ...
    py::class_<LIMITER>(m, "Limiter", "Limiter data")
        ...
        .def_property_readonly("PsiLim", [](LIMITER &l)
                               { return l.PsiMin; }, "Psi at limiting point")
        .def_property_readonly("RLim", [](LIMITER &l)
                               { return l.Xmin; }, "R at limiting point")
        .def_property_readonly("ZLim", [](LIMITER &l)
                               { return l.Zmin; }, "Z at limiting point")
        .def_property("Name", [](LIMITER &self)
                      { return self.Name; }, [](LIMITER &self, std::string name)
                      { strncpy(self.Name, name.c_str(), sizeof(LIMITER::Name) - 1); }, "Name of the limiter");
    ...
    py::class_<SUBCOIL>(m, "SubCoil", "Subcoil data")
        ...
        .def_property("Name", [](SUBCOIL &self)
                      { return self.Name; }, [](SUBCOIL &self, std::string name)
                      { strncpy(self.Name, name.c_str(), sizeof(COIL::Name) - 1); }, "Name of the subcoil");
    py::class_<ObjVecView<SUBCOIL>>(m, "SubCoils", "Subcoil vector view")
        ...
        .def("__setitem__", [](ObjVecView<SUBCOIL> &self, size_t i, SUBCOIL *c)
            ...
            self[i] = c; })
        .def("__len__", [](ObjVecView<SUBCOIL> &self)
             { return self.size(); })
        .def("new_subcoil", [](ObjVecView<SUBCOIL> &self)
             {
            SUBCOIL *c = new_SubCoil();
            return c; }, py::return_value_policy::reference_internal, "Add a new subcoil");

    py::class_<COIL>(m, "Coil", "Coil data and methods")
        .def(py::init(&new_Coil), "Create Coil")
        .def_readwrite("Enabled", &COIL::Enabled)
        .def_property("Name", [](COIL &self)
                      { return self.Name; }, [](COIL &self, std::string name)
                      { strncpy(self.Name, name.c_str(), sizeof(COIL::Name) - 1); }, "Name of the coil")
        .def_property_readonly("SubCoils", [](COIL &self) -> py::object
                               { return ObjVecView<SUBCOIL>::create(self.NumSubCoils, self.SubCoils); }, "Return vector of SubCoils")
        .def_property("NumSubCoils", [](COIL &self)
                      { return self.NumSubCoils; }, &set_NumSubCoils, "Number of subcoils, setting will erase old subcoils")
    ...
    py::class_<SUBSHELL>(m, "SubShell", "Subshell data")
        ...
        .def_property("Name", [](SUBSHELL &self)
                      { return self.Name; }, [](SUBSHELL &self, std::string name)
                      { strncpy(self.Name, name.c_str(), sizeof(SUBSHELL::Name) - 1); }, "Name of the subshell")
    ...
    py::class_<ObjVecView<SUBSHELL>>(m, "SubShells", "Subshell vector view")
        .def("__getitem__", &ObjVecView<SUBSHELL>::operator[],
             py::return_value_policy::reference)
        .def("__len__", [](ObjVecView<SUBSHELL> &self)
             { return self.size(); })
    ...
    py::class_<SHELL>(m, "Shell", "Shell data and methods")
        ...
        .def_property("Name", [](SHELL &self)
                      { return self.Name; }, [](SHELL &self, std::string name)
                      { strncpy(self.Name, name.c_str(), sizeof(SHELL::Name) - 1); }, "Name of the shell")
        ...
        .def_property_readonly("SubShells", [](SHELL &self) -> py::object
                               { return ObjVecView<SUBSHELL>::create(self.NumSubShells, self.SubShells); }, "Return vector of SubShells");

    py::class_<SEPARATRIX>(m, "Separatrix", "Separatrix data")
        .def(py::init(&new_Separatrix), "Create Separatrix")
        .def_property("Name", [](SEPARATRIX &self)
                      { return self.Name; }, [](SEPARATRIX &self, std::string name)
                      { strncpy(self.Name, name.c_str(), sizeof(SEPARATRIX::Name) - 1); }, "Name of the separatrix")
    ...
    py::class_<MEAS>(m, "Measure", "Measurement data")
        ...
        .def_property("Name", [](MEAS &self)
                      { return self.Name; }, [](MEAS &self, std::string name)
                      { strncpy(self.Name, name.c_str(), sizeof(MEAS::Name) - 1); }, "Name of the measurement")
        .def_property("Type", [](MEAS &self)
                      { return MeasType(self.mType); }, [](MEAS &self, MeasType type)
                      { self.mType = (int)type; }, "Type of the measurement")
        ...
        .def_property("Angle", [](MEAS &self)
                      { return self.parm.bp.Angle; }, [](MEAS &self, double angle)
                      { self.parm.bp.Angle = angle; }, "Angle")
        .def_property("Radius", [](MEAS &self)
                      { return self.parm.circle.Radius; }, [](MEAS &self, double radius)
                      { self.parm.circle.Radius = radius; }, "Radius")
        .def_property("Number", [](MEAS &self)
                      { return self.parm.circle.Number; }, [](MEAS &self, int number)
                      { self.parm.circle.Number = number; }, "Number")
        .def_property("CircleType", [](MEAS &self)
                      { return CircleType(self.parm.circle.CircleType); }, [](MEAS &self, CircleType ct)
                      { self.parm.circle.CircleType = (int)ct; }, "CircleType")
        .def_property("CoilNum", [](MEAS &self)
                      { return self.parm.coilcur.CoilNum; }, [](MEAS &self, int num)
                      { self.parm.coilcur.CoilNum = num; }, "CoilNum")
        .def_property("R1", [](MEAS &self)
                      { return self.parm.saddle.X1; }, [](MEAS &self, double x1)
                      { self.parm.saddle.X1 = x1; }, "Saddle Loop R1")
        .def_property("Z1", [](MEAS &self)
                      { return self.parm.saddle.Z1; }, [](MEAS &self, double z1)
                      { self.parm.saddle.Z1 = z1; }, "Saddle Loop Z1")
        .def_property("R2", [](MEAS &self)
                      { return self.parm.saddle.X2; }, [](MEAS &self, double x2)
                      { self.parm.saddle.X2 = x2; }, "Saddle Loop R2")
        .def_property("Z2", [](MEAS &self)
                      { return self.parm.saddle.Z2; }, [](MEAS &self, double z2)
                      { self.parm.saddle.Z2 = z2; }, "Saddle Loop Z2");
    ...
    py::class_<ObjVecView<COIL>>(m, "Coils", "Coil vector view")
        ...
        .def("__len__", [](ObjVecView<COIL> &self)
             { return self.size(); })
        .def("new_Coil", [](ObjVecView<COIL> &self, int n)
             { return new_Coil(n); }, py::return_value_policy::reference, "Add a new coil");
    py::class_<ObjVecView<LIMITER>>(m, "Limiters", "Limiter vector view")
        ...
        .def("__setitem__", [](ObjVecView<LIMITER> &self, size_t i, LIMITER *c)
            ...
            if (i<0 || i>=self.size())
                throw std::out_of_range("Index out of range");
            self[i] = c; })
        .def("__len__", [](ObjVecView<LIMITER> &self)
             { return self.size(); })
        .def("new_limiter", [](ObjVecView<LIMITER> &self)
             {
            LIMITER *c = new_Limiter();
            return c; }, py::return_value_policy::reference_internal, "Add a new limiter");
    py::class_<ObjVecView<SEPARATRIX>>(m, "Separatrices", "Separatrix vector view")
        ...
        .def("__setitem__", [](ObjVecView<SEPARATRIX> &self, size_t i, SEPARATRIX *c)
            ...
            if (i<0 || i>=self.size())
                throw std::out_of_range("Index out of range");
            self[i] = c; })
        .def("__len__", [](ObjVecView<SEPARATRIX> &self)
             { return self.size(); })
        .def("new_separatrix", [](ObjVecView<SEPARATRIX> &self)
             {
            SEPARATRIX *c = new_Separatrix();
            return c; }, py::return_value_policy::reference, "Add a new separatrix");
    py::class_<ObjVecView<MEAS>>(m, "Measures", "Measurement vector view")
        ...
        .def("__setitem__", [](ObjVecView<MEAS> &self, size_t i, MEAS *c)
            ...
            if (i<0 || i>=self.size())
                throw std::out_of_range("Index out of range");
            self[i] = c; })
        .def("__len__", [](ObjVecView<MEAS> &self)
             { return self.size(); })
        .def("new_meas", [](ObjVecView<MEAS> &self, MeasType mtype)
             {
            MEAS *c = new_Measure((int) mtype);
            return c; }, py::return_value_policy::reference, "Add a new measurement of type mtype");
    ...
    py::class_<ObjVecView<SHELL>>(m, "Shells", "Shell vector view")
        ...
        .def("__len__", [](ObjVecView<SHELL> &self)
             { return self.size(); })
        .def("new_shell", [](ObjVecView<SHELL> &self)
             { return new_Shell(0); }, py::return_value_policy::reference, "Add a new shell");
    ...
    py::class_<TOKAMAK>(m.attr("Machine"))
        ...
        .def(py::init(&FileInput), "Initialize Machine with a file",
             py::return_value_policy::reference)
        .def("__del__", [](TOKAMAK &self)
             { free_Tokamak(&self, FALSE); },
             "Free the machine") // this causes double free errors
        .def("init", [](TOKAMAK &self)
             { init_Tokamak(&self); }, "Initialize Machine")
        .def("set_coil_NumSubCoils", [](TOKAMAK &self, int i, int n)
             {
            if (self.Coils[i] != NULL)
                free_Coil(self.Coils[i], self.PsiGrid->Nsize);
            self.Coils[i] = new_Coil(n); }, "Set the number of subcoils for a coil")
        ...
        .def("read_restart", [](TOKAMAK &self)
             { ReadRestart(self.RSname, &self); }, "Read the restart file")
        .def("write_restart", [](TOKAMAK &self)
             { WriteRestart(self.RSname, &self); }, "Write the restart file")
        ...
        .def_readonly("PsiGrid", &TOKAMAK::PsiGrid, py::return_value_policy::reference_internal)
        .def_readonly("Plasma", &TOKAMAK::Plasma, py::return_value_policy::reference_internal)

        .def_property("Name", [](TOKAMAK &self)
                      { return self.Info; }, [](TOKAMAK &self, std::string name)
                      { strncpy(self.Name, name.c_str(), sizeof(TOKAMAK::Name) - 1); }, "Set the name")
        .def_property("Info", [](TOKAMAK &self)
                      { return self.Info; }, [](TOKAMAK &self, std::string info)
                      { strncpy(self.Info, info.c_str(), sizeof(TOKAMAK::Info) - 1); }, "Info about the machine")
        .def_property("Iname", [](TOKAMAK &self)
                      { return self.Iname; }, [](TOKAMAK &self, std::string name)
                      { strncpy(self.Iname, name.c_str(), sizeof(TOKAMAK::Iname) - 1); }, "Input name")
        .def_property("Oname", [](TOKAMAK &self)
                      { return self.Oname; }, [](TOKAMAK &self, std::string name)
                      { strncpy(self.Oname, name.c_str(), sizeof(TOKAMAK::Oname) - 1); }, "Output name")
        .def_property("Iname", [](TOKAMAK &self)
                      { return self.Iname; }, [](TOKAMAK &self, std::string name)
                      { strncpy(self.Iname, name.c_str(), sizeof(TOKAMAK::Iname) - 1); }, "Input name")
        .def_property("LHname", [](TOKAMAK &self)
                      { return self.LHname; }, [](TOKAMAK &self, std::string name)
                      { strncpy(self.LHname, name.c_str(), sizeof(TOKAMAK::LHname) - 1); }, "LH name")
        .def_property("MGname", [](TOKAMAK &self)
                      { return self.MGname; }, [](TOKAMAK &self, std::string name)
                      { strncpy(self.MGname, name.c_str(), sizeof(TOKAMAK::MGname) - 1); }, "MG name")
        .def_property("SGname", [](TOKAMAK &self)
                      { return self.SGname; }, [](TOKAMAK &self, std::string name)
                      { strncpy(self.SGname, name.c_str(), sizeof(TOKAMAK::SGname) - 1); }, "SG name")
        .def_property("SMname", [](TOKAMAK &self)
                      { return self.SMname; }, [](TOKAMAK &self, std::string name)
                      { strncpy(self.SMname, name.c_str(), sizeof(TOKAMAK::SMname) - 1); }, "SM name")
        .def_property("RSname", [](TOKAMAK &self)
                      { return self.RSname; }, [](TOKAMAK &self, std::string name)
                      { strncpy(self.RSname, name.c_str(), sizeof(TOKAMAK::RSname) - 1); }, "RS name")
        .def_property_readonly("Coils", [](TOKAMAK &self) -> py::object
                               { return ObjVecView<COIL>::create(self.NumCoils, self.Coils, &self, free_COIL); }, "Array of COILS")
        .def_property("NumCoils", [](TOKAMAK &self)
                      { return self.NumCoils; }, &set_NumCoils, "Number of coils, setting will erase old coils")
        ...
        .def_property_readonly("Limiters", [](TOKAMAK &self) -> py::object
                               { return ObjVecView<LIMITER>::create(self.NumLimiters, self.Limiters); }, "Get the limiters")
        .def_property("NumLimiters", [](TOKAMAK &self)
                      { return self.NumLimiters; }, &set_NumLimiters, "Number of limiters, setting will erase old limiters")
        .def_property_readonly("Shells", [](TOKAMAK &self) -> py::object
                               { return ObjVecView<SHELL>::create(self.NumShells, self.Shells, &self, free_SHELL); }, "Get the shells")
        .def_property("NumShells", [](TOKAMAK &self)
                      { return self.NumShells; }, &set_NumShells, "Number of shells, setting will erase old shells")
        .def("set_NumSubShells", &set_TOKAMAK_NumSubShells, "Set the number of subshells of shell i to n")
        .def_property_readonly("Measures", [](TOKAMAK &self) -> py::object
                               { return ObjVecView<MEAS>::create(self.NumMeasures, self.Measures, &self, free_MEAS); }, "Get the measurements")
        .def_property("NumMeasures", [](TOKAMAK &self)
                      { return self.NumMeasures; }, &set_NumMeasures, "Number of measurements, setting will erase old measurements")
        .def_property_readonly("Seps", [](TOKAMAK &self) -> py::object
                               { return ObjVecView<SEPARATRIX>::create(self.NumSeps, self.Seps); }, "Get the separatrixes")
        .def_property("NumSeps", [](TOKAMAK &self)
                      { return self.NumSeps; }, &set_NumSeps, "Number of separatrixes, setting will erase old separatrixes");

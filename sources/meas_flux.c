...
**	g_flux
**
**	The basic Greens function for a meas_flux.
**
*/
double        g_flux(double x, double z, double xc, double zc)
{

	return Green(x, z, xc, zc);
}
...
**
*/
void          meas_flux_Green(TOKAMAK * td, MEAS * m)
{
	PSIGRID      *pg;
	COIL         *c;
	SUBCOIL      *sc;
	...
	for (i = 0; i < ncoil; i++) {
		...
		for (isc = 0; isc < c->NumSubCoils; isc++) {
			sc = c->SubCoils[isc];
			sum += sc->CurrentFraction * g_flux(x, z, sc->X, sc->Z);
			if (pg->Symmetric)
				sum += sc->CurrentFraction * g_flux(x, z, sc->X, -(sc->Z));
		}
		CGrn[i] = sum;
	...
	for (ix = 1; ix < nmax; ix++)
		for (iz = 1; iz < nmax; iz++)
			PlGrn[ix][iz] = g_flux(x, z, pg->X[ix], pg->Z[iz]);
...
**
*/
void          meas_flux_Fit(TOKAMAK * td, MEAS * m)
{
	meas_mag_Fit(td, m);
}
...
**
*/
void          meas_flux_Now(TOKAMAK * td, MEAS * m)
{
	meas_mag_Now(td, m);
}
...
**
*/
void          meas_flux_L(TOKAMAK * td, MEAS * m, double *L)
{
	meas_mag_L(td, m, L);
}

...
def dipoleq_profiles(h5eq, npeak=1e20, eta=.67, NormalizeAtAxis=True):
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Create kinetic profiles from a dipoleq hdf5 file")
    parser.add_argument('h5files', metavar='h5file', type=str, nargs='+',
                        help="dipoleq hdf5 file(s)")
    parser.add_argument('--plot', '-p', action='store_true', 
                        help="Plot the profiles")
    parser.add_argument('--npeak', '-n', type=float, default=1e20,
                        help="Peak density")
    parser.add_argument('--eta', '-e', type=float, default=.67,
                        help="eta = dlnT/dlnn")
    parser.add_argument('--magX', '-x', action='store_true', default=True,
                        help="Start at magnetic axis")
    ...
    for h5file in args.h5files:
        ...
        with h5py.File(h5file) as h5f:
            ...
            if args.plot:
                plot_h5eq(h5f)
            psi, n, T = dipoleq_profiles(h5f,
                                         npeak=args.npeak,
                                         eta=args.eta,
                                         NormalizeAtAxis=args.magX)
